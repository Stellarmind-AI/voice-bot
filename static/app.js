...
async function playAudio(url) {
  ...
  const SILENCE_AFTER_SPEECH_MS = 1800; // ⬅️ slightly increased silence window
  const MIN_SPEECH_MS = 400;            // must speak at least 0.4s
  const RMS_START_THRESHOLD = 0.02;     // start talking threshold
  const RMS_CONTINUE_THRESHOLD = 0.012; // stay talking threshold (lowered)
  const MAX_UTTERANCE_MS = 30000;       // safety cap 30s
